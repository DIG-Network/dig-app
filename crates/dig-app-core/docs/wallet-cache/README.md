...
| `wallet-light-480.png` | 480 | 1.0 | 1232 × 2338 | `SHELL_MIN`. The wallet switcher takes the top band and the tab strip is the horizontal narrow-mode row BELOW it — the two do not touch, and neither is drawn over the other. Both custody badges are whole: `Your node cannot spend this` and `Spent without asking you`, with no ellipsis on either. `Your wallet` is the selected entry, and the card under the lead is headed `User wallet balance` rather than a bare `Balance`. The frame ends inside the `Offers` card — the pane continues below the window. |
...
The same Wallet tab with the OTHER wallet selected in the switcher (dig-app#339). Every one of these
is the `NotPublished` state, which is the state of every node in existence today: the node derives an
operator address and no control method publishes it, so dig-app says so rather than deriving a rival
copy of it.

| File | Logical width | Zoom | Physical | What is in the image |
| --- | --- | --- | --- | --- |
| `machine-light-960.png` | 960 | 1.0 | 2432 × 2488 | `This computer's wallet` is the selected switcher entry, carrying `Spent without asking you` — the §908 boundary stated permanently rather than on a page somebody has to find. The pane leads with `About this computer's wallet` (the explanation FIRST, because a second wallet is a surprise), then `Machine wallet address` stating why there is no address, then `Machine wallet balance`. **No numeral appears under either.** |
| `machine-dark-960.png` | 960 | 1.0 | 2432 × 2488 | The same in dark. Same three cards, same absence of a figure. |
| `machine-light-480.png` | 480 | 1.0 | 1232 × 2338 | `SHELL_MIN`. The switcher takes the top band and the tab strip sits BELOW it, clear of it. Both custody badges are drawn whole, and the user wallet's `Your node cannot spend this` is not clipped. The frame ends inside `Machine wallet balance`. |
| `machine-funded-light-960.png` | 960 | 1.0 | 2432 × 2488 | The machine wallet with a KNOWN address: it is shown in full, in mono, with its own **Copy** control, because funding it is the remedy and an address is what funding needs. The switcher entry now carries a fragment of it rather than a reason. `Machine wallet balance` reads **0 $DIG** and **0 XCH** — a zero that is honest here precisely because the address IS known, and it is explained rather than left bare: *"Your node's own wallet holds nothing, so it cannot pay collateral… Send $DIG to the address above to change that."* The card titles are what stop any of it being read as the person's own money. |
| `machine-funded-dark-960.png` | 960 | 1.0 | 2432 × 2488 | The same in dark. |
