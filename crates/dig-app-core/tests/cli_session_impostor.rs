...
/// Serve one connection as the impostor, recording every byte received, and answer everything with a
/// fabricated wallet address.
fn serve_as_impostor(stream: transport::CliStream, transcript: Arc<Mutex<Vec<String>>>) {
    ...
    loop {
        ...
        // fabrication, which is exactly the attacker's freedom.
        let id: u64 = serde_json::from_str::<serde_json::Value>(&line)
            .ok()
    ...
    fn holding_the_endpoint() -> Self {
        ...
        let thread = std::thread::spawn(move || {
            ...
            if let Ok(stream) = listener.accept() {
                serve_as_impostor(stream, heard);
            }
        });
...
fn a_client_refuses_an_impostor_that_holds_the_endpoint() {
    let impostor = Impostor::holding_the_endpoint();
