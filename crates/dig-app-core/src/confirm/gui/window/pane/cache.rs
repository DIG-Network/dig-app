...
/// Choosing a smaller limit can evict cached content, but nothing on this tab destroys anything a
/// person cannot get back — cached content is a copy of what the network still holds, and the node
/// asks before it evicts. A closed list, so a genuinely destructive verb added upstream has to be
/// classified here rather than inheriting the quiet treatment.
fn is_destructive(_: TrayAction) -> bool {
    false
}
