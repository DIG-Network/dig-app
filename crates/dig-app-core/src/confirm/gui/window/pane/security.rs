        ...
        let (height, hit) = card::interactive_card(
            ...
            |inner| {
                ...
                match account {
                    Some(kind) => {
                        inner.place(|ui, at| {
                            (text::body(ui, at, t, copy::security::protection(kind)), ())
                        });
                    }
                    // The loading state, for the same reason the Account pane draws one: nothing has
                    // reported yet, and "is my account safe right now" answered from the `Absent`
                    // default would tell a person with a locked account that they have none to
                    // protect. The wait is the only honest answer until the boot reports.
                    None => {
                        inner.place(|ui, at| {
                            (
                                state::banner(
...
impl Parts {
    ...
    ///
    /// The rows come from the shared [`super::actions_of`], which is also where the destructive
    /// registry and the occurrence counting live — so this sorts what every other pane renders,
    /// rather than assembling its own version of it.
    fn of(tab: &Tab) -> Self {
