...
fn holdings(balance: &BalanceReading) -> Vec<Readout> {
    match balance {
        ...
        BalanceReading::Unknown(why) => vec![Readout::new(
            copy::wallet::BALANCE_LABEL,
            Value::Unknown(unknown_reason(why)),
        )],
    }
}
...
/// Nothing the Wallet tab offers destroys anything: its verbs copy an address and open an
/// explanation. Stated as a closed list rather than left to a guess from the label, so a verb added
/// upstream that DOES destroy something has to be classified here rather than inheriting the quiet
/// treatment.
