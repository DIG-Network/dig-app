...
//! # The honesty type
//!
//! [`Value::Unknown`] is how a pane says a figure is absent: it takes the SENTENCE explaining why,
//! and draws in the unavailable colour rather than the value colour. dig_ecosystem#2326 ships tabs
//! as skeletons ahead of their data, and a skeleton showing a plausible zero is worse than an empty
//! pane, because nobody can tell it apart from a real reading.
//!
//! # What this type does and does not enforce
//!
//! It gives absence a FIRST-CLASS spelling, so writing an honest pane is easy and reviewing one is
//! mechanical: a figure is either derived from data or it is an `Unknown` carrying its reason.
//!
//! It does **not** make a dishonest pane impossible. `Value::Word("0")` compiles, and
//! [`Value::measure_bytes`] will happily render a zero it was handed. Making a placeholder
//! genuinely unexpressible needs private variants behind checked constructors — worth doing, and
//! tracked as dig_ecosystem#2337, but not true today. Do not read this type as a guarantee that
//! nothing on screen is invented; read it as the vocabulary that makes saying so the easy path.
