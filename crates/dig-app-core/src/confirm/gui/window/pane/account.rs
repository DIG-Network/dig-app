...
use super::flow::Flow;
use super::identity;
use super::status::is_destructive;
use super::text;
...
fn state_card(flow: &mut Flow, t: &Tokens, facts: &PaneFacts) {
    let Some(kind) = facts.account else {
        return;
    };
    flow.place(|ui, at| {
        (
            card::card(ui, at, t, None, |inner| {
                inner.place(|ui, at| {
                    (
                        data::badge(ui, at.left_top(), t, kind.word(), kind.tone()).height(),
                        (),
                    )
                });
                inner.gap(space::S3);
                inner.place(|ui, at| (text::body(ui, at, t, copy::account::summary(kind)), ()));
            }),
            (),
        )
    });
...
fn grouped(tab: &Tab) -> (Vec<Group>, Vec<Group>) {
    ...
    let groups: Vec<Group> = tab
        ...
        .map(|section| {
            let actions =
                super::actions_in(section.rows.iter().cloned(), &mut seen, &is_destructive);
            Group {
                heading: section.heading.clone(),
                destroys: actions.iter().any(|action| is_destructive(action.id)),
                actions,
            }
        })
        .collect();
