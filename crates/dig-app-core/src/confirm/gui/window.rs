...
mod tests {
    ...
    /// Photograph EVERY screen of the first-run DID wizard, in BOTH themes.
    ///
    /// ```text
    /// cargo test -p dig-app-core --lib -- --ignored --nocapture did_wizard_gallery
    /// ```
    ///
    /// # Why it is separate from [`prompt_gallery`]
    ///
    /// These eight screens are ONE journey — dig_ecosystem#2341's — and they are read as a sequence:
    /// a person meets the funding claim, then the offer that spends, then a wait, then exactly one of
    /// five endings. Kept beside the consent prompts they would be shuffled among windows that have
    /// nothing to do with them, and the wizard shipped with no captures at all precisely because it
    /// had nowhere of its own to live.
    ///
    /// Every screen is built by the journey's OWN builder. Re-typing the copy here would produce a
    ...
    fn did_wizard_gallery() {
        let dir = std::path::PathBuf::from(
            std::env::var("DIG_WIZARD_SHOTS").unwrap_or_else(|_| "target/wizard-shots".into()),
        );
        std::fs::create_dir_all(&dir).expect("the gallery directory");

        for (name, screen) in wizard_gallery() {
            for theme in [Theme::Light, Theme::Dark] {
                let label = match theme {
                    Theme::Light => "light",
                    Theme::Dark => "dark",
    ...
    /// Every screen of the DID wizard, named for its file.
    fn wizard_gallery() -> Vec<(&'static str, Screen)> {
        use crate::account::did::MintEvidence;
        use crate::account::journey::{
            funding_claim, mint_offer, mint_report, waiting_claim, waiting_screen, WizardNotice,
        };
        use crate::account::mint::{MintOutcome, WaitProgress, GIVE_UP_AFTER_SECS};
        use crate::confirm::{NoticePrompt, QrArt};

        /// A mainnet-shaped receiving address, so the code and the mono line are photographed at the
        /// real length. It is a fixture, not anyone's address.
        ...
        /// A full-length spend id, since its wrapping is part of what a screenshot is checking.
        ...
        // A report is a one-button notice, so it is photographed as one — the same shape
        // `dialog_gallery` shows it in.
        let report = |notice: WizardNotice| {
            confirm(ConfirmContent::notice(&NoticePrompt {
                title: notice.title,
        ...
        // The wait's own two-button check-in, rebuilt from `waiting_screen` exactly as `WindowedWait`
        // does — one button would be a picture of a screen that cannot be stopped.
        let check_in = |unreachable_looks: u32| {
            let screen = waiting_screen(&WaitProgress {
                elapsed_secs: WAITED_SECS,
                give_up_after_secs: GIVE_UP_AFTER_SECS,
                unreachable_looks,
            });
            confirm(ConfirmContent::claim(&waiting_claim(&screen)))
        };

        let art = QrArt::encode(ADDRESS);
        vec![
            (
                "fund",
                confirm(ConfirmContent::claim(&funding_claim(ADDRESS, art.as_ref()))),
            ),
            ("offer", confirm(ConfirmContent::claim(&mint_offer()))),
            ("waiting", check_in(0)),
            // Six consecutive failed looks is what `connection_lost` reads as offline, so this is the
            // check-in that admits it cannot see the chain rather than one that pretends it can.
            ("waiting-offline", check_in(6)),
            (
                "pending",
                report(mint_report(
