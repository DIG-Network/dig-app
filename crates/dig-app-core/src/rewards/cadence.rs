...
///
/// Three cases, not `Option<f64>`: `entry_count == Some(0)` is real post-eviction data and must
...
/// reassuring reading) instead of the honest "we do not yet know the mirror set". A three-case enum
/// keeps SPEC 2.3's rule that every state be expressible exactly once (see also
...
///
/// `daily_funding_base_units == 0` also answers [`CadenceReading::NoMirrorsYet`]: a zero funding
/// rate has no cadence to state either, and a caller must not read a number out of it. This
...
mod tests {
    ...
    /// A zero funding rate has no cadence either, and must not be confused with a genuine mirror
    /// count of zero in what it TELLS the reader, even though both currently answer the same
    /// variant: neither is a number, which is the property this test pins.
    #[test]
    fn zero_funding_rate_is_not_a_number() {
        assert_ne!(days_between_claims(Some(5), 0), CadenceReading::Days(0.0));
    }
}
