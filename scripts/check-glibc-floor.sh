...
#
# WHY this gate exists (dig-app#238, dig_ecosystem#1736): dig-node v0.64.0's `linux-x64` binary
# silently began requiring
...
#        3 = the binary is not a glibc build at all (a *-musl target), which SPEC.md §9 rejects;
...
if [ -z "$binary" ] || [ -z "$floor" ]; then
  echo "usage: $(basename "$0") <binary> <max-glibc-version>   e.g. dist/dig-app-1.2.3-linux-x64 2.31" >&2
  exit 2
fi
if [ ! -f "$binary" ]; then
...
# see: the pinned container only fixes what the binary is built AGAINST (which a musl
# target built inside that container still satisfies), and a start-on-an-old-distro check passes because a
# musl binary starts everywhere. Accepting it here would let "just target musl and delete the
# container" pass every gate, silently changing the libc under every published artifact.
#
...
check-glibc-floor: FAIL — $binary requires NO versioned glibc symbols.
  ...
  SPEC.md §9 declares the -gnu target for every published Linux artifact, and a silent swap to musl
  changes name resolution, thread-stack sizes and locale handling under a binary users already
  trust, while making this gate, the pinned container and the floor all vacuous. Build for -gnu, or set
...
check-glibc-floor: FAIL — $binary requires glibc $highest, above the supported floor $floor.
  ...
  the workflow, SPEC.md §9 and the docs together.
EOF
  exit 1
fi
