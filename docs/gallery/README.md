...
| `content-store-rewards-<state>-light-960.png` | the Content tab with one store row's Rewards section OPEN, in each of its five states — `waiting`, `not-answerable`, `unreachable`, `empty`, `ready`. `not-answerable` is the state a real install shows today, on every store row: no released dig-node serves the read that maps a store to a distributor, so it answers `-32601` and the section says so in the recessed treatment rather than in amber. Every frame contains the whole section, card border and all, so nothing is cut off below the last sentence — `ready` is the only one with a money figure in it (`Paid … in total to mirrors`), and its figures are a stated FIXTURE: no released dig-node answers `dig.listRewardDistributors`, so a live distributor cannot be photographed, and what the picture is evidence about is the shipping formatter and the layout, not the numbers |
