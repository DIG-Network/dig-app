...
1. **Bootstrap unlock** — a DIGOP1 password **the user knows and types** (§3.1e). It is never
   generated by the app, never stored anywhere, and never derivable from the machine. It opens the
   account's sealed **master seed** — the sole at-rest secret.
...
SINGLE account **master seed**, sealed in a per-user file backend under the user's own bootstrap-unlock
password (§3.1e), and from which every profile's
...
**At-rest storage precedence (bootstrap unlock).** The sealed seed is a FILE on every platform, and
its password comes from the user on every platform (§3.1e). There is no credential-store tier: an OS
credential store gates access per-application, which protects a secret from other applications but not
from anything running as the user — and it cannot gate a secret behind something the user knows at all.
A machine-held password is therefore not a custody control, and dig-app does not keep one.

1. **Sealed file (the only tier)** — the sealed blob is a file
...
Unlock **fails closed**: a wrong password, a tampered blob, or a foreign key yields an opaque error that
never distinguishes the cause and never produces partial plaintext. That indistinguishability is a
REQUIREMENT, not an accident of implementation: an error that told a caller "the password was wrong, and
the blob is fine" would confirm to an attacker that guessing is the productive attack. The user-facing
message consequently names both possibilities and guesses at neither (§3.1e).

### 3.1e The unlock password (normative)

Unlocking a DIG Account MUST require a secret supplied by the USER at unlock time.

- **The app MUST NOT generate, derive, or store the unlock password.** No CSPRNG-generated password, no
  OS credential-store entry, no keyring, no file, no machine-derived value (a host id, a TPM-sealed blob,
  a DPAPI/`SecItem` wrapper). The password exists only in the user's head and in the [`Zeroizing`] buffer
  it is typed into.
- **There MUST be no zero-prompt unlock path and no fallback that needs no password.** This includes
  a "remember me on this device" convenience: it is explicitly NOT provided, because a fallback that
  needs no password is indistinguishable, to an attacker with code running as the user, from having no
  password at all. Re-introducing one is a **breaking change to this section**, not a feature.
- **The app starts LOCKED.** No account is opened at process start. Consequently the APP-SIGN loopback
  listener (§5.6) is not merely refusing while locked — it is **not bound**, so no seed exists in the
  process for a paired client to reach. The user unlocks deliberately, from the tray.
- **A re-auth after a lock MUST collect the password again.** The idle auto-lock, `Lock now` and the OS
  screen lock drop key material; a sign that follows one MUST prompt. A re-auth that could re-derive the
  seed without the user would make every lock decorative.
- **Unlock gates SIGNING, SEALING and REVEALING only — never READING content.** Per the ecosystem's
  frictionless-consumption rule, no custody state may gate consuming data. Reading DIG content goes
  client→dig-node and involves no account at all; the app's node connection, engine and gateway hold no
  account handle and are assembled before any account exists. Every locked-state surface (the tray
  details window, the unlock prompt, `dign account status`) MUST say so, because a user who believes a
  locked account has broken DIG will go looking for a workaround.
- **Strength: LENGTH ONLY**, at least 12 characters. Composition rules ("one digit, one symbol") MUST NOT
  be imposed — they push users to short, guessable patterns, and NIST SP 800-63B abandoned them.
- **A new password MUST be typed twice and compared** before a seed is sealed under it. A typo in a
  password nobody has yet is not a retry; it is an account sealed under a string the user cannot know.
- **The recovery phrase MUST be shown and confirmed BEFORE the password becomes load-bearing** (§3.1a).
  This ordering is normative for account creation, for a restore, and for the migration below: a
  forgotten password costs the account unless the words exist first.
- **Every prompt MUST be masked by default and offer a deliberate reveal** (§3.1d), and MUST be drawn
  in-process by the OS-native input window — never by a subprocess helper, which a `PATH` impostor could
  replace.
- **A wrong password MUST fail closed**, leave the sealed seed untouched, and be re-askable. It MUST NOT
  be reported in a way that distinguishes it from an unreadable blob (see §3.1's fail-closed rule); the
  user-facing message names both causes and points at the remedy for the second.

**Migration from a machine-held password (normative).** An account sealed under the retired
machine-generated password MUST be re-sealed IN PLACE under a user-chosen one, never destroyed: read the
stored password, open the account, re-confirm the recovery phrase, collect the new password, then delete
and re-enrol from the SAME phrase, and finally delete the credential entry. The seed is unchanged, so the
identity key, addresses, per-profile DEKs and every already-sealed blob still open. Where a re-seal is
impossible — no vaulted phrase, or the stored password does not open the account — the account MUST be
left EXACTLY as it is and the user routed to the always-available replace verbs (§3.1c). A migration MUST
NOT be the thing that destroys an account, and a failure after the point of no return MUST be reported,
never swallowed.
...
phrase*. The phrase is the ONE portable custody root: the sealed seed blob is openable only with the
password its owner chose on that machine (§3.1e), so the words are the only thing a user can carry to a
new machine — and, since the password can be forgotten as well as the machine lost, the only thing that
recovers an account at all.
...
- **Say the true state.** An account with no recovery phrase MUST be labelled as such in the account
  ...
  currently qualify: setting up an account on a host whose account surface is not wired yet (§3.1c), and
  revealing the recovery phrase while the account is locked (whose remedy, `Unlock…`, is directly above it).
...
rather than a gap — the Linux account surface is not wired yet (§3.1c), so it is always the unsupported-host
state and no destroy window can be drawn there. Wiring Linux MUST NOT land without addressing it. (Since
§3.1e the unlock password is a prompt rather than a credential-store entry, so nothing technical blocks that
wiring any more — only the rest of the account surface, notably `discard_account`, being wired with it.)
...
The bootstrap password is collected from the USER at an OS-native masked prompt on every platform
(§3.1e) — asked once to unlock an account that exists, and twice (plus the length bar) to choose the
password a new one is sealed under. There is no boot-time unlock: the app starts locked and the signing
channel stays down until the user unlocks from the tray. The rest of the Linux account surface is still
unwired (§3.1c), so the account boot yields no residency there (dig_ecosystem#962).
...
- **Threat model (summary).**
  ...
  - **At-rest theft of a raw disk artifact yields only DIGOP1 ciphertext** — the sealed file's bytes
    are ciphertext and its password is never persisted anywhere, on any platform (§3.1e). This is
    stronger than it was before dig_ecosystem#1817, and the difference is the whole point of that
    change: the unlock password used to be machine-generated and filed in the OS credential store, so
    the real access control was that store's per-application ACL and the DIGOP1 sealing was
    defense-in-depth beneath it rather than an independent secret. Now the password is a secret the
    user holds, so the ciphertext is protected by something no amount of local access reveals.
  - **Code running as the user can no longer reach the master seed on demand.** Previously it could:
    the credential store releases its entries to the logged-in session without a prompt, so anything
    running as U1 could read the password and open the seed with no user interaction at all. Now the
    seed is absent from the process until the user types a password, and a prompt is something an
    attacker's code cannot type. What remains in scope is narrower and unavoidable — see the accepted
    cases below.
  ...
  - **Accepted (out of scope):** malware running AS U1 can read the master seed out of dig-app's
    memory **while the account is unlocked**, and can keylog the password as it is typed. Those are
    the-user-is-the-user cases and no unlock scheme defeats them. What the password buys is that the
    window is bounded rather than permanent: before an unlock, and after the idle auto-lock, `Lock now`
    or an OS screen lock, there is no seed in the process to read and no stored password to fetch — so
    an attacker must be resident AND lucky in timing, rather than merely resident. A live-session SYSTEM
    compromise likewise sees that session's in-memory key only while it is unlocked and attached.
...
| Threat | Mitigation |
|---|---|
| **Auto-sign** — a local process silently drives a sign | The native confirm + biometric is mandatory on every sign (the production `SignPolicy`; no default-allow); loopback-only bind + Host/`Origin`/token-MAC guards reject an unpaired caller. And while the account is locked the listener is **not bound at all** (§3.1e), so there is no channel to drive and no seed in the process to drive it toward. |
| **Signing from a lock the user believes is a lock** — a sign arriving after an idle auto-lock | The re-auth gate collects the user's password again (§3.1e). Before dig_ecosystem#1817 it re-unlocked from the credential store, so every lock was decorative: it dropped key material the next signature silently re-derived. |
